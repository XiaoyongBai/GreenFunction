...
void MathOperationT::PrintMatrix(int NRow, int NCol, const complex<double>* matrix, std::string minfo)
{
    cout << "MATRIX\t"<<minfo<<"::"<<endl;
    const complex<double>* temp=matrix;
    ...
    for(int i=0; i<MD; i++)
    {
        for(int j=0; j<MD; j++)
        {
            *temp=0;
            ...
            for(int l=0; l<MD; l++)
            {
                *temp+=matrixA[i*MD+l]*matrixB[j*MD+l];
            }
...
{
    double* mat1=new double[MD*MD];
    
    multAB(MD, MD, matrixA, MD, MD, matrixB, mat1);
    
    multABT(MD, mat1, matrixA, resultMatrix);
